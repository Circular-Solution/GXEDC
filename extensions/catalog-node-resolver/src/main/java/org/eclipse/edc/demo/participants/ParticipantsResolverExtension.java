...
public class ParticipantsResolverExtension implements ServiceExtension {
    ...
    public void initialize(ServiceExtensionContext context) {
        ...
        if (!participantListFile.exists()) {
            monitor.warning(
                    "Path '%s' does not exist. It must be a resolvable path with read access. Will not add any VCs."
                            .formatted(participantsPath));
        }
    }
    ...
    public TargetNodeDirectory createLazyTargetNodeDirectory() {
        if (nodeDirectory == null) {
            nodeDirectory = new LazyLoadNodeDirectory(
                    typeManager.getMapper(), participantListFile, didResolverRegistry, monitor);
        }
    ...
    public TargetNodeFilter skipSelfNodeFilter(ServiceExtensionContext context) {
        ...
        return targetNode -> {
            ...
            if (!predicateTest) {
                monitor.debug(
                        "Node filter: skipping node '%s' for participant '%s'"
                                .formatted(targetNode.id(), participantId));
            }
