...
1. Creates the consumer and provider participant contexts in their Identity Hubs. Each participant's signing keypair (Ed25519, verification method `#key-1`) is generated by the Identity Hub, stored in the participant's vault, and published in its DID document - no key file needed.
