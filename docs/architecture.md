...
Gaia-X compliance credentials (`gx:LabelCredential`) are obtained from a GXDCH out-of-band and loaded into each participant's Identity Hub via the Identity API - see `seed.sh` (`GX_JWT` env var, `just seed <jwt>`). The former in-process GXDCH proxy (`gx-issuer`) has been removed.
